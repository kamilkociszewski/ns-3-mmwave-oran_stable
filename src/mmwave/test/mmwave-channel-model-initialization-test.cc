...
MmwaveThreeGppChannelInitializationTestCase::DoRun (void)
{
  ...
  for (auto ccIndex : mmWaveEnbDev->GetCcMap ())
  {
    ...
    Ptr<ThreeGppUmaPropagationLossModel> threeGppPlm = DynamicCast<ThreeGppUmaPropagationLossModel> (plm);

    // be sure the MmWaveHelper has created the proper propagation loss model
    if (!threeGppPlm)
    {
      NS_TEST_ASSERT_MSG_EQ (false, true, "The propagation loss model is not properly configured");
    }
    ...
    if (!threeGppSplm)
    {
      NS_TEST_ASSERT_MSG_EQ (false, true, "The spectrum propagation loss model is not properly configured");
    }
    ...
    NS_TEST_ASSERT_MSG_EQ (phyFreq, freq [ccIndex.first], "The operating frequency of the PHY layer has not been properly configured");
    NS_TEST_ASSERT_MSG_EQ (phyFreq, plmFreq.Get (), "The operating frequency of the propagation loss model has not properly configured");
    DoubleValue splmFreq;
    threeGppSplm->GetChannelModelAttribute ("Frequency", splmFreq);
    NS_TEST_ASSERT_MSG_EQ (phyFreq, splmFreq.Get (), "The operating frequency of the spectrum propagation loss model has not properly configured");
    ...
    Ptr<ThreeGppUmaChannelConditionModel> threeGppCcm = DynamicCast<ThreeGppUmaChannelConditionModel> (ccm);
    if (!threeGppCcm)
    {
      NS_TEST_ASSERT_MSG_EQ (false, true, "The channel condition model is not properly configured");
    }
    ...
    NS_TEST_ASSERT_MSG_EQ (ccm, threeGppSplmCcm, "The channel condition model associated to the propagation loss model is not the same as the one associated to the spectrum propagation loss model");
  }
}
