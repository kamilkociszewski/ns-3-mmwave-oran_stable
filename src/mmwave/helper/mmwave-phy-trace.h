...
private:
  ...
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin1UeSpecific; //!< UE specific TX with SINR < -6dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin2UeSpecific; //!< UE specific TX with SINR -6dB to 0 dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin3UeSpecific; //!< UE specific TX with SINR 0 dB to 6 dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin4UeSpecific; //!< UE specific TX with SINR 6 dB to 12 dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin5UeSpecific; //!< UE specific TX with SINR 12 dB to 18 dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin6UeSpecific; //!< UE specific TX with SINR 18 dB to 24 dB
  std::map<RntiCellIdPair_t, uint32_t> m_macSinrBin7UeSpecific; //!< UE specific TX with SINR > 24 dB
   ...
   * @param key
   */
  uint32_t GetMapValue (std::map<RntiCellIdPair_t, uint32_t> map, 
