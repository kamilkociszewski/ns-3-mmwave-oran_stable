...
{
  static TypeId tid = TypeId ("ns3::MmWaveEesmCc")
...
double
MmWaveEesmCc::ComputeSINR (const SpectrumValue& sinr, const std::vector<int>& map, uint8_t mcs,
                         uint32_t sizeBit,
                         const MmWaveErrorModel::MmWaveErrorModelHistory &sinrHistory) const
{
  ...
  for (uint32_t i = 0; i < historySize; ++i)
    {
      Ptr<MmWaveEesmErrorModelOutput> output = DynamicCast<MmWaveEesmErrorModelOutput> (total.at(i));
