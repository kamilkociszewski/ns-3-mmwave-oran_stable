...
*/
        ...
        static TypeId
                ...
                TypeId("ns3::LteEnbNetDevice")
                        ...
                        .AddAttribute("e2andLogging",
                                      ...
                                      BooleanValue(false),
                                      MakeBooleanAccessor(&LteEnbNetDevice::m_e2andlog),
                                      MakeBooleanChecker())
    ...
    LteEnbNetDevice::DoInitialize(void) {
        ...
        UpdateConfig();
        std::map < uint8_t, Ptr < ComponentCarrierEnb > > ::iterator
        it;
...
//                }
                if (m_forceE2FileLogging ||
                    m_e2andlog) { // give some time for the simulation to start, TODO check value
                    m_cuUpFileName = "cu-up-cell-" + std::to_string(m_cellId) + ".txt";
                    std::ofstream csv{};
                    csv.open(m_cuUpFileName.c_str());
                    csv << "timestamp,ueImsiComplete,DRB.PdcpSduDelayDl (cellAverageLatency),"
    ...
    LteEnbNetDevice::BuildRicIndicationMessageCuUp(std::string plmId) {
        ...
        for (auto ue: ueMap) {
            ...
            NS_LOG_DEBUG(
                    Simulator::Now().GetSeconds() << " " << std::to_string(m_cellId) << " cell, connected UE with IMSI "
                                                  << imsi
        ...
        NS_LOG_DEBUG(Simulator::Now().GetSeconds() << " " << std::to_string(m_cellId) << " cell, connected UEs number "
                                                   << ueMap.size()
        ...
        NS_LOG_DEBUG(
                Simulator::Now().GetSeconds() << " " << std::to_string(m_cellId) << " cell volume " << cellDlTxVolume);
