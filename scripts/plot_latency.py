...
tstFileDlVar['var 100us sf'] = ['DlPdcpStats_50ue_100sf_500B_var.tr','DlPdcpStats_100ue_100sf_500B_var.tr','DlPdcpStats_120ue_100sf_500B_var.tr']#,'DlPdcpStats_130ue_100sf_500B_var.tr']
...
#tstFileUlVar['var 100us sf'] = ['UlPdcpStats_50ue_100sf_500B_var.tr','UlPdcpStats_100ue_100sf_500B_var.tr','UlPdcpStats_120ue_100sf_500B_var.tr']
...
tstFileDlFixed['fixed 100us sf'] = ['DlPdcpStats_50ue_100sf_500B_fixed.tr','DlPdcpStats_100ue_100sf_500B_fixed.tr','DlPdcpStats_120ue_100sf_500B_fixed.tr']#,'DlPdcpStats_130ue_100sf_500B_fixed.tr']
...
for tst in tstFileDlVar :
    ...
    tstDatDl[tst] = []
    for f in files:
        cmd = 'grep DlRxPDU data/' + f + ' | awk \'{ if($8 < 20000000)  total += $8 } END { print total/NR }\''
        c = os.popen(cmd)
...
for tst in tstFileUlVar :
    ...
    tstDatUl[tst] = []
    for f in files:
        cmd = 'grep UlRxPDU data/' + f + ' | awk \'{ if($8 < 20000000) total += $8 } END { print total/NR }\''
        c = os.popen(cmd)
        res = float(c.read().rstrip())
        print str(res),
...
yticks = [0,1,5,10,25]
tstListDl = ['fixed 100us sf','var 100us sf']
istyle = 0
fig = plt.figure()
ax = fig.add_subplot(111)
for tst in tstListDl:
    ax.semilogy(numFlows,tstDatDl[tst],lineStyle[istyle],linewidth=2.0,markersize=10.0,label=tst)
    istyle = istyle + 1
...
#     ax2.semilogy(numFlows,tstDatUl[tst],lineStyle[istyle],label=tst)
#     istyle = istyle + 1
# ax2.legend(bbox_to_anchor=(0.4, 1.0))
# ax2.set_xlabel('Num UL flows')
# ax2.set_ylabel('Mean PDCP latency (ms)')
# #plt.yticks(yticks)
# #plt.title('')
# # pylab.text(0, 0, txJitterLabel, fontsize=12)
