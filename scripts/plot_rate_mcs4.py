...
csvFileList = ['fading_harq_all.tr','fading_harq_all.tr']
datList = [];
lineSkip = -1;
...
#                                                   np.mean(mcsDat.rate[ b*nBucketElem:min((b+1)*nBucketElem,len(mcsDat.rate)) ]))
            avgDatList[i].sinrAvg = np.convolve(mcsDat.sinr[::-1], np.ones(nBuckets)/nBuckets, 'valid')
            avgDatList[i].rateAvg = np.convolve(mcsDat.rate[::-1], np.ones(nBuckets)/nBuckets, 'valid')
...
#         f1 = interp1d(np.unique(avgDatList[i].sinrAvg[firstEl:numEl]), np.unique(avgDatList[i].rateAvg[firstEl:numEl]),kind="linear")
#         xnew = np.linspace(f1.x.min(), f1.x.max(), num=4*numEl)
